...
export const ProductCard: React.FC<ProductCardProps> = ({ product, onOpenDetails }) => {
  return (
    <motion.div
      ...
      className="group flex flex-col bg-white overflow-hidden border border-stone-200/50 shadow-sm hover:shadow-md transition-all duration-300 rounded-lg"
    >
      {/* Image container with hover overlays */}
      <div className="relative overflow-hidden aspect-[3/4] bg-stone-100 cursor-pointer" onClick={() => onOpenDetails(product)}>
        <img
          src={product.image}
          alt={product.name}
      ...
      <div className="p-4 sm:p-5 flex flex-col flex-1">
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-1 text-amber-500">
            ...
            <span className="text-[10px] font-semibold text-stone-600 font-sans">{product.rating}</span>
          </div>
        </div>
        ...
        <h3
          ...
          className="font-serif text-base font-semibold text-stone-950 hover:text-stone-700 transition-colors cursor-pointer leading-snug line-clamp-1 mb-2"
        >
          {product.name}
        </h3>
